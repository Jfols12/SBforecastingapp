...
# --- User Input CPI Adjustment ---
st.sidebar.header("User Input")
cpi_adjust = st.sidebar.slider("CPI Adjustment (%)", -5.0, 5.0, 0.0, 0.1)
adjusted_cpi = df['cpi'].copy() * (1 + cpi_adjust / 100)

# --- Forecast with Transactions + CPI (ARIMAX) ---
endog = df['revenue']
exog = df[['transactions']].copy()
exog['adj_cpi'] = adjusted_cpi

model1 = SARIMAX(endog, exog=exog, order=(1,1,1))
results1 = model1.fit(disp=False)

forecast_exog = exog.iloc[-4:]
