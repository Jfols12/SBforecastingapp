...
txn_input = st.sidebar.slider("Enter expected Transactions", 800, 1200, 1000, step=10)

# --- Create adjusted CPI value ---
adjusted_cpi_value = latest_cpi * (1 + cpi_input / 100)

# --- Train ARIMAX model on historical data ---
endog = df['revenue']
exog = df[['transactions', 'cpi']]

model1 = SARIMAX(endog, exog=exog, order=(1, 1, 1))
results1 = model1.fit(disp=False)

# --- Forecast with user inputs (next 4 quarters) ---
future_dates = pd.date_range(start=df.index[-1] + pd.DateOffset(months=3), periods=4, freq='Q')
future_exog = pd.DataFrame({
    'transactions': [txn_input] * 4,
    'cpi': [adjusted_cpi_value] * 4
}, index=future_dates)

forecast = results1.get_forecast(steps=4, exog=future_exog)
